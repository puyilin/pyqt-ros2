...
class ControlApp(QMainWindow):
    def __init__(self):
        ...
        #self.initUi()  
        self.ControlGui()
        # self.ARIE_view.setStyleSheet("border: 0px")
    ...
    def show_camera(self):
        ...
        # 显示原图
        show = cv2.resize(self.image, (148, 120))  # 把读到的帧的大小重新设置为 640x480
    ...
    def show_processed_image(self):
        ...
        if self.Ui_main.ProcessingImage.text() == "关闭图片":
            # 处理图片
            # flag, frame1 = cv2.flip(self.image, 1)
            flag, frame1 = self.cap.read()
            gray = cv2.cvtColor(frame1, cv2.COLOR_BGR2GRAY)

            # 显示处理的图片
            show_1 = cv2.resize(gray, (148, 120))  # 把读到的帧的大小重新设置为 640x480
    ...
    def parameterview(self):
        if self.Ui_main.ParameterEstimate.text() == "参数估计":
            if self.Ui_main.PartType.currentText() == "圆轴圆孔":
                ...
                self.Ui_main.ParameterView_2.setText("半径")
            else:
                self.Ui_main.ParameterView_1.setText("中心位置")
